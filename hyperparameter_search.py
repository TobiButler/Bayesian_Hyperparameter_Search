...
Purpose: This module contains methods that can be used to perform a bayesian style hyperparameter search when provided with previous hyperparameter sets and associated log-likelihood 
evaluations (as a csv file). In order to utilize the search, a user must call this module's fit() method and provide it with several arguments. Two of these arguments include a string 
"filename" and a Callable "cv_function". The "cv_function" should be a custom cross validation function that takes a dict of hyperparameter:value pairs and returns a log-likelihood value 
as an evaluation of the hyperparameters. The string "filename" must correspond to a csv file containing comma separated sets of hyperparameters and their log-likelihood evaluation.
...
produced, we determine the direction of minimum variance along which to search (it is the principle component of the covariance matrix with the smallest singular value). We assume that 
log-likelihood values will change most rapidly along this least principle component. To search along this component, we adapt a binary search algorithm. We search outwards in either 
direction, evaluating the log-likelihood of the associated hyperparameter values, until the log-likelihood stops increasing. Then, we pick the largest hyperparameter set along this 
direction and search between it and the closest evaluated hyperparameter sets on either side. These closest sets are reffered to as LEFT and RIGHT. We repeat this process, choosing the 
largest hyperparameter set and evaluating on either side of it (between the most-likely set and LEFT, and between the most-likely set and RIGHT) until our convergence requirements are met. 
Convergence is reached when one of two conditions are satisfied on EACH side of the most-likely hyperparameter set. The first condition is for the log-likelihood to decrease as we search 
between the most-likely set and one of the two closest sets to it along the search direction (LEFT and RIGHT). The second condition is for the derivative of the log-likelihood evaluations 
to appear to be decreasing. We say that this condition has been satisfied if extending the log-likelihood evaluated at LEFT OR RIGHT by the derivative on that side produces a greater 
log-likelihood than that associated with the most-likey set (suggesting that the derivative must decrease before we get to the most-likely set). The derivative on each side of the most-likely 
point is taken to be the average derivative between LEFT/RIGHT and the set next closest to LEFT/RIGHT on that same side. These convergence conditions are designed to end the search if we no 
longer expect to make significant progress along the current search-direction.
...
the standard deviation of the recently computed covariance matrix in this direction and call it v. This is our initial seach vector. and call it v. Given ||v|| = 1, we evaluate the 
...
def fit(csv_filename: str, cv_function: Callable, hyperparameter_names: set, constant_hyperparameters:set = set({}), integer_hyperparameters: set = set({}), 
        list_hyperparameters:set = set({}), search_base:float = 2, initial_dirichlet_process_prior:float = 1, num_rounds:int=1, minimum_log_likelihood: float = -np.inf, 
        minimum_hyperparameter_variance:float = 1e-10, print_status:bool = False, dtype: type = np.float64, **kwargs):
    """
    Parameters:
    ----------
        ...
        for n in range(num_rounds):
            ...
            while True:
                ...
                if left_likelihood >= right_likelihood and left_likelihood >= mid_likelihood: 
                    ...
                    right_likelihood = _minimal_covariance_sampling(cv_function, right_dirichlet_process_prior, hyperparameter_samples_container, csv_filename, hyperparameter_names, constant_hyperparameters, integer_hyperparameters, non_constant_hyperparameters, list_hyperparameters, list_amounts, minimum_log_likelihood, minimum_hyperparameter_variance, search_base, print_status, **kwargs)
...
def _minimal_covariance_sampling(cv_function:Callable, dirichlet_process_prior:float, hyperparameter_samples_container:list[pd.DataFrame], csv_filename:str, hyperparameter_names:set, 
        ...
        search_base:float, print_status:bool, dtype:type, **kwargs):
    """
    ...
    prior_log_likelihoods = (-1/2) * np.sqrt(np.linalg.slogdet(cov)[1]) + (-1/2) * t.sum(t.abs(t.matmul(diffs, np.linalg.inv(cov)) * diffs), axis=1)[:,None]
    prior_likelihoods = np.exp(30 - np.max(prior_log_likelihoods))
    ...
    original_log_likelihoods:np.ndarray = hyperparameter_samples_container[0]["log_likelihood"][hyperparameter_samples_container[0]["log_likelihood"] > minimum_log_likelihood].to_numpy()
    adjusted_log_likelihoods = original_log_likelihoods - np.max(original_log_likelihoods)
    ...
    likelihoods = (likelihoods/np.sum(likelihoods))[:,None]
    ...
    directional_derivatives = (mean_likelihood-original_log_likelihoods) / np.sqrt(np.einsum("ij,ij->i", diffs, diffs)) * diffs
    
    # divide each directional derivative by the prior_likelihoods and by the inverse covariance matrix. This reduces bias from frequently sampled points and directions.
    search_direction = np.sum(np.dot(directional_derivatives, np.linalg.inv(cov)) / prior_likelihoods)
    
    # apply covariance matrix so that we are initially searching one standard deviation away from the mean
    search_direction = np.dot(search_direction / np.linalg.norm(search_direction), cov)

    if print_status: print("MINIMUM VARIANCE SEARCH DIRECTION: " + str(search_direction))
...
def _evaluate_likelihood(cv_function:Callable, hyperparameter_sample:np.ndarray, samples_table_container:list[pd.DataFrame], csv_filename:str, hyperparameter_names:set, 
    constant_hyperparameters:set, integer_hyperparameters:set, non_constant_hyperparameters:set, list_hyperparameters:set, list_amounts:list, print_status:bool, **kwargs):
    """
    ...
    csv_table = copy.deepcopy(samples_table_container[0])
    if list_amounts is not None: # convert expanded hyperparameters back to "list" type objects
        ...
        for key, num in list_amounts:
            csv_table[key] = csv_table.iloc[:,num_non_list_non_constant_hyperparameters:num_non_list_non_constant_hyperparameters+num].apply(list, axis=1) # log-likelihood should be in column -1
            csv_table = pd.concat([csv_table.iloc[:,:num_non_list_non_constant_hyperparameters],csv_table.iloc[:,num_non_list_non_constant_hyperparameters+num:]], axis=1)
        temp_series = csv_table["log_likelihood"]
        csv_table = csv_table.drop(["log_likelihood"], axis=1)
        csv_table["log_likelihood"] = temp_series
    csv_table[list(integer_hyperparameters)] = csv_table[list(integer_hyperparameters)].round(0).astype("int64")
    csv_table.to_csv(csv_filename) # overwrite previous csv file 
